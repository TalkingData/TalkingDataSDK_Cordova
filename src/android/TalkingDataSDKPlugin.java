...
public class TalkingDataSDKPlugin extends CordovaPlugin {
    private Context ctx;
    ...
    public boolean execute(String action, JSONArray arguments, CallbackContext callbackContext) {
        if (action.startsWith("getDeviceId")) {
            getDeviceId(callbackContext);
        } else if (action.startsWith("getOAID")) {
            getOAID(callbackContext);
        } else {
            try {
                Class<TalkingDataSDKPlugin> classType = TalkingDataSDKPlugin.class;
